    ...
    public static class AnalyticsCalculator
    {
        ...
        private static IReadOnlyList<AnalyticsRecordRow> GetByFormat(IReadOnlyList<AnalyticsEvent> events) =>
            events
                ...
                .Select(group => (Format: group.First().Format, Row: CreateEventRow(group.First().Format?.Name ?? EventService.NO_FORMAT_NAME, group)))
        ...
        private static IReadOnlyList<DeckFormatRow> GetDeckFormats(IReadOnlyList<AnalyticsEvent> events) =>
            events
                ...
                .Select(group =>
                {
                    ...
                    return new DeckFormatRow(row.Label, format?.Name ?? EventService.NO_FORMAT_NAME, format?.StartDate, row.EventCount, row.Record);
                })
