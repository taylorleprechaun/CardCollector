...
- **Decks** &mdash; deck lists pasted from DuelingBook (*Download Deck*, then the YDKe code or the contents of the YDK file). Use **Add deck** on an event, or **Import deck** when the event already has a decklist link; the deck is linked to that event, and optionally to every other event that has the same decklist link and no deck yet. You can also point an event at a deck you already imported, or remove the link. The deck viewer shows every card as an image grouped into Main (rows of 10), Extra, and Side (one row of 15 each), with the main deck&rsquo;s monster, spell, and trap counts, and links each card to its detail page. Cards are always shown in one standard order, whatever order the list was pasted in: main deck monsters, then fusion, synchro, Xyz, and link monsters, then spells, then traps, alphabetically within each group (so Main is monsters, spells, traps; Extra is fusion, synchro, Xyz, link; Side is all of them in that order). Decks can be renamed or deleted from the Decks page; deleting one keeps its events and only clears their link
