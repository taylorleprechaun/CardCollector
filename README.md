...
- **Events** &mdash; tournaments you&rsquo;ve played, with date, location, event type, deck, decklist link, finish, players, top cut, and notes; add, edit, and delete them from the Tournaments menu. Filter by date range, format, event type, location, and deck. Each event&rsquo;s **details** page shows its format (with that format&rsquo;s top strategies), its round-by-round matches, and the match, game, and dice-roll records. Rounds are added, edited, and deleted right on that page, built for quick entry between matches on a phone: the next round label is pre-filled, opponent decks autocomplete from earlier rounds, common scores are one tap (with a Custom score option for anything else), and the result follows the score until you override it. If an event&rsquo;s rounds are ever out of order, the page offers a one-click sort
- **Decks** &mdash; deck lists pasted from DuelingBook (YDKe code or YDK file), linked to the event they came from (and optionally other events sharing its decklist link). The viewer shows every card as an image under Main/Extra/Side, sorted by type then name, with monster/spell/trap counts and a Forbidden/Limited/Semi-Limited badge overlaid on each restricted card &mdash; checked against the banlist in effect at that event or the current one, toggled top-right, with a picker for other lists or events. Decks can be renamed or deleted from the Decks page
- **Analytics** &mdash; records and win % filtered by format, deck, opponent, date, event type, and location. Tabs for deck &times; format, matchups, a per-format trend chart, deck, format, event type, and dice roll
...
<!-- coverage:start -->
![C# Tests](https://img.shields.io/badge/C%23%20tests-1359%20passing-brightgreen)
![C# Coverage](https://img.shields.io/badge/C%23%20coverage-93%25-brightgreen)
![JS Tests](https://img.shields.io/badge/JS%20tests-268%20passing-brightgreen)
...
- Tournament data lives in the SQLite database, not the card caches. An event&rsquo;s format is never stored: it is worked out from the event&rsquo;s date and the format date ranges, so editing a format&rsquo;s dates re-classifies its events. Filtering events by format is a date-range filter for the same reason. A match&rsquo;s result (`W`, `L`, or `T`) is stored as recorded rather than computed from the game score, so a manual override is kept. A round&rsquo;s score allows at most two wins and two losses (ties are unlimited, since an end-of-match tie is rare but possible), a bye is always stored as a win with no games or dice roll, a new round is placed by its label (numbered rounds, then Top 8, Top 4 and Finals) so a deleted round added back returns to its place, and deleting a round renumbers the rounds after it. Decklist links only accept `http` and `https` addresses.
- Analytics uses each round&rsquo;s stored result. Win % is `(W + T/2) / (W + L + T)`. Byes count unless excluded; matchups and dice records never include them.
- DuelingBook deck pages can&rsquo;t be fetched from a link, so decks are imported by pasting only. A YDKe code or YDK file lists cards by passcode. Passcodes are mapped to the app&rsquo;s card ID before a deck is stored: 17 cards have a different primary passcode in yaml-yugi than in YGOProDeck (for example Monster Reborn), and a card with alternate artwork has an extra passcode per artwork, so all of those resolve to the yaml-yugi ID. A passcode the card data doesn&rsquo;t know is still kept and shown as an unknown card. A deck stores each distinct card once per section with a quantity and its position in the pasted list. An event&rsquo;s deck link has no database foreign key, so deleting a deck clears the link on its events in the same transaction.
- TCG Forbidden &amp; Limited data comes from [yaml-yugi-limit-regulation](https://github.com/DawnbrandBots/yaml-yugi-limit-regulation), not YGOProDeck (it only has the current list and disagrees with yaml-yugi on a few cards). Lists are keyed by Konami ID (mapped via yaml-yugi&rsquo;s `konami_id`; a card without one is treated as unrestricted) and fetched from `BanlistSettings:BaseUrl`/`IndexUrl`, cached for 7 days (`BanlistSettings:CacheTtlDays`) with a stale-cache fallback. Two lists are recorded under a regional date rather than the AMER date they took effect on; `BanlistSettings:EffectiveDateOverrides` corrects those.
